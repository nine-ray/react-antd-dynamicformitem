...
class DynamicFormItem extends React.Component {
    ...
    componentDidMount() {
        const {keysName, getFieldValue} = this.props;
        const keys = getFieldValue(`${keysName}`);
        if (keys.length === 0) {
            this.add();
        };
    ...
    render() {
        const {keysName, getFieldDecorator, getFieldValue, extra, renderViewData, lable, renderView, addButtonStyle,addView, ...other} = this.props;// 有默认传来的 chang事件，和 value值
        ...
        return (
            <Row>
                ...
                <FormItem {...formItemLayoutWithOutLabel}>
                    <Button type="dashed" onClick={this.add} style={{width: '60%',...addButtonStyle}}>
                        {addView?addView: <view><Icon type="plus"/>Add field</view>}
                    </Button>
                </FormItem>
            </Row>
        );
